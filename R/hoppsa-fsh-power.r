...
############################################################
##base scenario
## truncated normal age at entry distribution
agedist<- floor(rnorm(100000,45,6))
agedist<-agedist[agedist>37]
...
##run simulation
set.seed(myseed)
params$n_sims<-10000 ## 10k replicates of simulation
...
############################################################
## if age U(40-50)
params$ages <-  seq(40,50,1), # ages of study entry
## set seed reproducibility
set.seed(myseed)
## run sims
params$n_sims<-ncores*10 ## quick run for insight
